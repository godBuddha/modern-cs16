...
namespace Veldrid
{
    ...
    public class ImGuiRenderer : IDisposable
    {
        ...
        {
            // .NET đặt tên theo Namespace.Folder.filename — tìm theo suffix
            string[] allNames = _assembly.GetManifestResourceNames();
            ...
            if (matchedName == null)
                throw new InvalidOperationException($"Could not find embedded resource matching: {resourceName}. Available: {string.Join(", ", allNames)}");
            using (Stream s = _assembly.GetManifestResourceStream(matchedName))
            {
